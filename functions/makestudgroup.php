...
else
{
	...
	if($status)
	{
		...
		if($status_2==false)
		{
			header("Location: ./../addstudentgroup.php?prev=partial&partialfailmsg=$errorstr_2");	
		}
		else
		{
			header("Location: ./../addstudentgroup.php?prev=done");
		}
	}
	else
	{
		mysqli_rollback($db);
		mysqli_close($db);
		header("Location: ./../addstudentgroup.php?prev=fail&msg=$errorstr");
	}
	;
}
?>
