...
else
{
	...
	if($status)
	{
		...
		if($status_2==false)
		{
			header("Location: ./../addfacgroup.php?prev=partial&partialfailmsg=$errorstr_2");	
		}
		else
		{
			header("Location: ./../addfacgroup.php?prev=done");
		}
	}
	else
	{
		mysqli_rollback($db);
		mysqli_close($db);
		header("Location: ./../addfacgroup.php?prev=fail&msg=$errorstr");
	}
	;
}
?>
