...
else
{
	...
	if($status)
	{
		mysqli_commit($db);
		mysqli_close($db);
		header("Location: ./../studentgroups.php?prev=done&msg=Student groups deleted successfully");
	}
	else
	{
		mysqli_rollback($db);
		mysqli_close($db);
	//	echo "$errorstr";
		header("Location: ./../studentgroups.php?prev=fail&error=Couldn't delete student groups: $errorstr");
	}
	;
}
?>
